...
# ClawAgent Setup - main automation script (single-agent fork of ClawFactory).
...
# ClawAgent v1.0 ships with OpenClaw 2026.4.27 - the version manually
...
$LogDir                = Join-Path $env:ProgramData 'ClawAgent'
...
$WslUser               = 'clawuser'
$GatewayPort           = 8787
$FirewallRuleName      = 'ClawAgent-Block-Inbound-8787'
...
$RunOnceRegName        = 'ClawAgentResumeInstall'
...
$ProviderConfig = @{
    grok = @{
        ...
        CredentialTarget = 'ClawAgent/GrokApiKey'
        AllowlistHosts   = @('api.x.ai')
        KeySource        = 'windows-credential-manager:ClawAgent/GrokApiKey'
        Endpoint         = 'https://api.x.ai/v1'
    }
    openai = @{
        DisplayName      = 'OpenAI (ChatGPT)'
        DefaultModel     = 'gpt-5'
        CredentialTarget = 'ClawAgent/OpenAIApiKey'
        AllowlistHosts   = @('api.openai.com')
        KeySource        = 'windows-credential-manager:ClawAgent/OpenAIApiKey'
        Endpoint         = 'https://api.openai.com/v1'
    }
    claude = @{
        DisplayName      = 'Anthropic Claude'
        DefaultModel     = 'claude-sonnet-4-6'
        CredentialTarget = 'ClawAgent/AnthropicApiKey'
        AllowlistHosts   = @('api.anthropic.com')
        KeySource        = 'windows-credential-manager:ClawAgent/AnthropicApiKey'
        Endpoint         = 'https://api.anthropic.com/v1'
    }
    gemini = @{
        DisplayName      = 'Google Gemini'
        DefaultModel     = 'gemini-2.5-pro'
        CredentialTarget = 'ClawAgent/GeminiApiKey'
        AllowlistHosts   = @('generativelanguage.googleapis.com')
        KeySource        = 'windows-credential-manager:ClawAgent/GeminiApiKey'
...
function Install-WslDistroWithFallback {
    ...
    if ($BundledRootfs -and (Test-Path -LiteralPath $BundledRootfs)) {
        $WslInstallDir = 'C:\Program Files\ClawAgent\WSL'
...
function Invoke-WslInstallWithRestart {
    ...
    Show-RestartDialog -Title 'ClawAgent - Restart Required' -Message (
        "WSL2 needs to be installed. Your computer will restart to complete this step.`r`n`r`n" +
        'ClawAgent will resume automatically after restart.'
    )
    ...
    try {
        ...
        Write-Log ERROR "Restart failed: $($_.Exception.Message). Reboot manually; ClawAgent will resume on next login."
        exit 0
    }
}
...
function Step-EnsureWsl {
    ...
    Set-ItemProperty -Path $RunOnceRegPath -Name 'ClawAgentResume' -Value $runOnceVal -Type String
    Write-Log INFO "Reboot required. RunOnce key registered: $runOnceVal"
    Save-Checkpoint 'EnsureWsl'
    if (-not (Test-IsSilent)) {
        Add-Type -AssemblyName System.Windows.Forms
        [System.Windows.Forms.MessageBox]::Show(
            "WSL2 requires a restart to complete setup.`nClawAgent will continue automatically after restart.`nClick OK to restart now.",
            'Restart Required',
...
function Step-ConfigureWslConfig {
    ...
    try {
        ...
        $needsShutdown = $false
        $banner    = '# Added by ClawAgent v1.0 - keeps WSL VM alive so the gateway stays running.'
                ...
                if ($currentValue -eq '-1') {
                    ...
                    if (-not (Test-IsSilent)) {
                        try {
                            ...
                            [System.Windows.Forms.MessageBox]::Show(
                                "Your .wslconfig has a vmIdleTimeout setting that is different from ClawAgent's recommended value (-1).`r`n`r`nCurrent: $currentValue`r`nRecommended: -1`r`n`r`nWe did not modify your file. ClawAgent will install but the gateway may stop unexpectedly when WSL is idle.`r`n`r`nTo fix: edit $WslConfigPath and set vmIdleTimeout=-1, then restart your machine.`r`n`r`nSee README.md for details.",
                                'ClawAgent Setup - Manual Action Recommended',
                                [System.Windows.Forms.MessageBoxButtons]::OK,
                    ...
                    if (-not $shownDialog) {
                        ...
                        # 'ClawAgent:' prefix is the hard-fail signature - the
                        # outer catch in this function re-throws on that prefix
                        # so Invoke-WithRollback can run its rollback path.
                        throw 'ClawAgent: .wslconfig conflict detected during silent install. See log for fix instructions.'
                    }
                    $needsShutdown = $false
                }
            }
            ...
            if ($otherDistros.Count -gt 0 -and $proceedShutdown) {
                ...
                try {
                    ...
                    $choice = [System.Windows.Forms.MessageBox]::Show(
                        "ClawAgent needs to restart WSL to apply the .wslconfig change. The following WSL distros are running:`r`n  $list`r`n`r`nContinuing will shut down ALL running WSL distros. Save any work in those distros before clicking OK.`r`n`r`nClick Cancel to skip - the .wslconfig change will take effect when WSL next idles.",
                        'ClawAgent Setup - Restart WSL?',
        ...
        if ($_.Exception.Message -like 'ClawAgent:*') {
...
"@
    ...
    if ($script -notmatch '/usr/sbin/nft') {
        ...
        throw 'ClawAgent: firewall script validation failed'
    }
...
You are the default chat assistant for the ClawAgent operator. Reply directly and concisely to whatever the operator asks. Keep answers under three sentences unless explicitly asked for detail.
AGENT
    else
...
function Step-ConfigureOpenClaw {
    ...
    # (v1.0.3 audit). install.sh sets OPENCLAW_UPDATE_IN_PROGRESS=1 before its
...
function Step-CreateAgentDirectories {
    # ClawAgent v1.0.0: single-agent variant. OpenClaw creates `main` automatically
    # during install; this step is a defensive idempotent ensure. The factory
    # variant (ClawFactory) creates 4 sub-agent dirs here (orchestrator/scout/
    # builder/publisher); ClawAgent intentionally has only `main`.
    Write-Log INFO 'Step 10: Ensuring main agent workspace dir exists.'
    $script = "set -e`nmkdir -p /home/clawuser/.openclaw/agents/main`necho `"main agent dir ready`""
    $rc = Invoke-WslBash -Script $script -User $WslUser
    if ($rc -ne 0) { Write-Log WARN "Failed to create main agent dir (exit=$rc)" }
...
function Step-WindowsFirewallDeny {
    ...
    New-NetFirewallRule `
        ...
        -Profile     Any `
        -Description 'ClawAgent gateway must never be reachable from the network.' | Out-Null
...
function Step-RegisterWslHostTask {
    ...
    Write-Log INFO 'Step 16: Registering ClawAgent WSL Host task (keeps gateway alive during idle).'
    $TaskName = 'ClawAgent WSL Host'
...
if ($Resume) {
    ...
    Write-Log INFO "==== ClawAgent Setup - resuming after restart (provider=$Provider) ===="
    Write-Host ''
    ...
    Remove-ResumeFlag
    Write-Log INFO "==== ClawAgent Setup - starting (provider=$Provider) ===="
}
...
Invoke-WithRollback {
    ...
    Step-CreateAgentDirectories  # ClawAgent: ensure single 'main' agent dir exists
...
Write-Log INFO '==== ClawAgent Setup - completed successfully ===='
Remove-ResumeFlag
Remove-RunOnceResume

Write-Host ''
Write-Host 'SUCCESS. Your hardened ClawAgent is ready.' -ForegroundColor Green
